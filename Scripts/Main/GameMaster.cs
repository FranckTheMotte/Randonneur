...
public partial class GameMaster : Node
{
    ...
    //Player Data Methods
    public static void SavePlayerData(int slotNum)
    {
        Save(SaveTypes.playerDat, slotNum);
    }

    public static void LoadPlayerData(int slotNum)
    {
        Load(SaveTypes.playerDat, slotNum, false);
    }

    public static void LoadPlayerDataSlot(int slotNum)
    {
        Load(SaveTypes.playerDat, slotNum, true);
    }

    public static void DeletePlayerData(int slotNum)
    {
        Delete(SaveTypes.playerDat, slotNum);
    }
    ...
    {
        Load(SaveTypes.gameDat, 1);
    }

    public static void DeleteGameData()
    {
        Delete(SaveTypes.gameDat, 1);
    }
    ...
    private static void Load(SaveTypes mySaveType, int slotNum, bool loadToSlot = false)
    {
        ...
        if (FileAccess.FileExists(myFilePath) == false)
        {
            ...
            {
                GD.Print("(GameMaster) File doesnt exist: " + myFilePath);
            }
    ...
    private static void initializeSlot(SaveTypes mySaveType, int slotNum)
    {
        if (mySaveType == SaveTypes.playerDat)
        {
            ...
            if (slotNum == 1)
            {
                loadedPlayerDataSlot1 = new PlayerData();
                SavePlayerData(slotNum);
            }
            if (slotNum == 2)
            {
                loadedPlayerDataSlot2 = new PlayerData();
                SavePlayerData(slotNum);
            }
            if (slotNum == 3)
            {
                loadedPlayerDataSlot3 = new PlayerData();
                SavePlayerData(slotNum);
            }
        }
        if (mySaveType == SaveTypes.gameDat)
